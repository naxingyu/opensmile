  ...
  if (x[xl-1] == 's' || x[xl-1] == 'S') {
    ...
    if (isSec!=NULL) {
      if (*isSec == -1) *isSec = 1;
        ...
        SMILE_IERR(2,"mixing time specifiers in seconds and frames is not supported! Please ensure that all time values are not suffixed by an 's' ! (openSMILE in this case will treat all values as being given in input level frames ...)");
      }
    }
  } else {
    if (isSec!=NULL) {
