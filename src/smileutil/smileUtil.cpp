...
void smileDsp_impulse_response_gaussFadeout(sSmileDspImpulseResponse *ir,
  ...
  if (ir != NULL) {
    ...
    float sum = 0.0;
    for (float n = -Nf2 + 1.0; n < Nf2; n += 1.0) {
      float e = n/(sigmaNf2);
...
sSmileDspImpulseResponse *smileDsp_impulse_response_init(int N, sSmileDspImpulseResponse *s = NULL) {
  if (N <= 0) {
    return NULL;
  }
  if (s == NULL) {
...
void smileDsp_sincGauss_impulse_response(sSmileDspImpulseResponse *ir,
    float cutoffHz, float Ts, float sigma=0.5, bool highpass=false, bool fadeout=true) {
  if (ir != NULL) {
    if (cutoffHz < 1.0) {
      cutoffHz = 1.0;
    }
...
void smileDsp_gammatone_impulse_response(sSmileDspImpulseResponse *ir,
    float fc, float bw, float Ts, float a = 1.0, int order = 1, bool fadeout=true) {
  if (ir != NULL) {
    for (int n = 0; n < ir->N; n++) {
      ir->h[n] = a * Ts * pow((float)n, (float)(order - 1)) * exp(-2.0 * M_PI * bw * n * Ts) * cos(2.0 * M_PI * fc * n * Ts);
    }
    if (fadeout) {
...
// with center frequency fc and bandwidth bw for a sampling period of Ts
// set N in ir before calling this function!
void smileDsp_gabor_impulse_response(sSmileDspImpulseResponse *ir, float fc, float bw, float Ts, bool fadeout=true) {
  if (ir != NULL) {
    ...
    float Nf = (float)ir->N;
    FLOAT_DMEM *h = ir->h;
    for (float n = -Nf/2.0 + 1.0; n < Nf/2.0; n += 1.0) {
...
sSmileDspConvolverState *smileDsp_block_convolve_init(int blocksize, sSmileDspImpulseResponse *ir,
    sSmileDspConvolverState *s = NULL) {
  if (s == NULL) {
...
void smileDsp_block_convolve_destroy(sSmileDspConvolverState * s,
    bool noFreeIr = false, bool noFreePtr = false) {
  if (s != NULL) {
...
void smileDsp_block_convolve(sSmileDspConvolverState *s, FLOAT_DMEM *x,
    FLOAT_DMEM *y, int N, int outputStep = 1, int outputOffset = 0) {
  if (N < s->blocksize) {
