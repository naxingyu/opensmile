...
namespace openSMILEbatchGUI
{
    public partial class Form1 : Form
    {
        ...
        {
            workDirectory.Text = "."; // System.IO.Directory.GetCurrentDirectory();
            refreshConfigFiles();
        }
...
//                p.StartInfo.FileName = smilePath;
        ...
        {
            string binary = workDirectory.Text + "\\SMILExtract_Release.exe";
            logfile = workDirectory.Text + "\\smile.log";
            string options = "-C \"" + workDirectory.Text + "\\config\\" + configurations.Text + "\" ";
            options += "-logfile \"" + logfile + "\" ";
            string wavFilename = filename;
            options += "-I \"" + wavFilename + "\" ";
            options += "-instname \"" + Path.GetFileName(wavFilename) + "\" ";
            string arffOutFile = arffOutName.Text;
            string csvOutFile = csvOutName.Text;
            string lldCsvOutFile = lldCsvOutName.Text;
            if (haveArffOut.Checked)
            {
                options += "-O \"" + arffOutFile + "\" ";
                if (arffAppend.Checked)
                {
                    //options += "-appendfuncsarff 1 ";
                }
                else
                {
                    options += "-appendfuncsarff 0 ";
                }
            }
            else
            {
            ...
            if (haveCsvOut.Checked)
            {
                ...
                {
                    //options += "-appendfuncscsv 1 ";
                }
                else
                {
                    options += "-appendfuncscsv 0 ";
                }
            }
            else
            {
            ...
            {
                options += "-lldoutput \"" + lldCsvOutFile + "\" ";
            ...
            if (runSmileBinary(workDirectory.Text, binary, options) == 0)
            {
                prepareUiAfterAnalysisRun();
            }
        }
